...
func exit() {
	ioutil.WriteFile("proc", []byte("0") ,0644)
	os.Exit(2)
}

func main() {
	//tell stat 
	ioutil.WriteFile("proc", []byte("1") ,0644)
	...
	set := Config{Tone: []string{"██", "▓▓", "▒▒", "░░", "  "}, ResizeMul: "100", SavePath: "out/"} //default setting
	...
	fmt.Println(" SavePath:", set.SavePath, "\n")

	//get picpath
	pathbyt, _ := ioutil.ReadFile("picpath")
	picpath := string(pathbyt)
	fname := filepath.Base(picpath)
	os.Remove("picpath")
	...
	html, err := os.Create(set.SavePath+"text.html")
	if err != nil {
		fmt.Println("err:", err)
		os.Exit(2)
	}
	...
	htmlimg, err := os.Create(set.SavePath+fname+".html")
	if err != nil {
		fmt.Println("err:", err)
		exit()
	}
